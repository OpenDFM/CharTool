...
def compute_data_metrics(batch: DataProto, use_critic: bool = True) -> dict[str, Any]:
    ...
    if batch.non_tensor_batch["acc"] is not None:
        acc = torch.from_numpy(batch.non_tensor_batch["acc"]).squeeze(-1)
        format_acc = torch.from_numpy(batch.non_tensor_batch["format_acc"]).squeeze(-1)
    else:    
        acc = None
        format_acc = None
    ...
    if acc is not None:
        non_aborted_acc = acc[non_aborted_mask]
        non_aborted_format_acc = format_acc[non_aborted_mask]
        # print(f"non_aborted_acc.shape {non_aborted_acc.shape}, non_aborted_format_acc.shape {non_aborted_format_acc.shape}")

    acc_mean = torch.mean(non_aborted_acc).detach().item() if acc is not None else None
    format_acc_mean = torch.mean(non_aborted_format_acc).detach().item() if format_acc is not None else None
    ...
    metrics = {
        ...
        # accuracy
        "critic/acc/mean": acc_mean,
        "critic/format_acc/mean": format_acc_mean,
        # adv
